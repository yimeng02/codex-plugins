...
description: Connect Codex to ZenTao Open Source 21.7.1, collect required first-run connection details from the user, derive MCP tools from the logged-in member's groups, provision the developer-default daily 07:00 new-Bug scan, create complete cross-session Bug solution packages, perform confirmation-gated writes, and require a real ZenTao UI screenshot after every successful write. Use when the user mentions 禅道, ZenTao, a ZenTao URL, initial configuration, account credentials, project Bug/需求/任务, member roles, MCP setup or diagnostics, scheduled new-Bug detection, source-backed Bug analysis, solution-package handoff, cache refresh or screenshots after writes, write confirmation policy, or asks Codex to read or operate ZenTao items.
---
...
4. If credentials are missing, stop and ask the user for all required first-configuration inputs: ZenTao site address, member account, member password, and explicit allowed project IDs. Also ask whether an outer HTTP Basic layer exists; if yes, collect its account and password. Do not guess, inherit, silently default, or scrape missing values from unrelated files. Run `python3 <plugin-root>/scripts/configure.py set` only after the user supplied the required values. Enter passwords through the hidden interactive prompts; never echo them or place them in a command, log, screenshot, source file, or Git. Use `import-mcp` only when the user explicitly asks to reuse a named legacy connector after reviewing the redacted source profile.
